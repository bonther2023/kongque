...
class AgentSettlement extends AbstractCronTask
{
...
//        return '*/1 * * * *';//每分钟执行一次
    ...
    function run(int $taskId, int $workerIndex)
    {
        ...
        $setting = settings();
        //结算代理
        $agentLists = Agents::create()->field('balance,id,username,name,bank,card')
            ->where('status',Agents::STATUS_1)
            ->where('name','', '<>')
            ->where('bank','', '<>')
            ->where('card','', '<>')
            ->where('balance',$setting['trade_money'],'>=')
            ->all();
        foreach ($agentLists as $agentItem){
